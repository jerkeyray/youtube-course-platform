...
export default function UserMenu() {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {dropdownOpen && (
        <div id="user-menu" className="absolute right-0 mt-2 w-48 rounded-lg bg-background p-2 shadow-lg border">
          <div className="mb-2 border-b px-2 pb-2">
            <div className="font-medium">{user.fullName ?? "User"}</div>
            <div className="truncate text-sm text-muted-foreground">{user.primaryEmailAddress?.emailAddress}</div>
          </div>
